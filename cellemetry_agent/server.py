...
def check_model_cache():
        ...
        if path.exists():
            ...
            if snapshots_dir.exists():
                ...
                if snapshots:
                    ...
                    logger.info(f"✅ Found cached model at: {cache_info['snapshot_path']}")
                    return cache_info
    ...
    if not cache_info["exists"]:
        logger.warning(f"⚠️ Model cache not found. Checked: {cache_info['all_checked_paths']}")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if cache_info.get("snapshot_path"):
            logger.info(f"📦 Loading from local cache: {cache_info['snapshot_path']}")
            try:
                ...
                logger.info("✅ Loaded from explicit cache path")
        ...
        # Method 2: Let transformers find it automatically (works with HF_HOME)
            ...
            logger.info("☁️ Loading with transformers auto-discovery: facebook/sam3")
            ...
            logger.info("✅ Loaded via transformers auto-discovery")
        ...
        logger.warning("⚠️ Running without SAM3 model (Mock mode)")
